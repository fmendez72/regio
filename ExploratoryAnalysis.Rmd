...
Here we fit a mixed effects ordinal regression. There is a lot more going on in the analysis so this has to be read more carefully in the link below. Nonetheless, the basic point remains that further thought is needed on potential country attributes to include in the model when country patterns are by and large very similar.

Here is an example of a better model that includes a Net Payer interaction, which is significant. But goes the wrong way theoretically - net recipients more likely to be negative (panel 1), and marginally less positive (3).
